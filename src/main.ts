...
} from '@becomes/cms-client/handlers';
import type {
    BCMSApiKeyAccess,
    BCMSApiKeySignature,
    BCMSClient,
...
import {
    createBcmsClientSecurity,
    errorWrapper,
} from '@becomes/cms-client/util';
        ...
        if (signature) {
            ...
            if (conf.query) {
                ...
                for (const key in conf.query) {
                    queryPairs.push(
                        `${key}=${encodeURIComponent(conf.query[key])}`,
                    );
                }
